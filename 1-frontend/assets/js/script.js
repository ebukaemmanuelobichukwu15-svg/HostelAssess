(() => {
  ...
  async function initAdminManagement() {
    ...
    const load = async () => {
      try {
        const [{ admins, invites }, { hostels }, { settings: sessionSettings }] = await Promise.all([API.request('/admins'), API.request('/hostels/manage?all=true'), API.request('/admins/academic-session')]);
        ...
        const currentEnd = Number(sessionSettings.currentSession.split('/')[1]);
        const nextSession = `${currentEnd}/${currentEnd + 1}`;
        const scheduled = sessionSettings.scheduledSession
          ? `<div class="session-scheduled"><strong>${escapeHtml(sessionSettings.scheduledSession)}</strong><span>Activates ${formatDate(sessionSettings.scheduledStartAt)}</span><button type="button" class="table-action danger-link" data-cancel-session>Cancel schedule</button></div>`
